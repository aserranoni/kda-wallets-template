...
export class WalletConnect extends Connector {
  ...
  private getDataFromSession(session: any) {
    switch (KADENA_NETWORK_ID) {
      case 'mainnet01':
        return this.parseAccountData(session.namespaces.kadena.accounts[0]);
      case 'testnet04':
        return this.parseAccountData(session.namespaces.kadena.accounts[1]);
      case 'development':
        return this.parseAccountData(session.namespaces.kadena.accounts[2]);
      default:
  ...
  public async signTx(command: PactCommandToSign): Promise<PactSignedTx> {
    ...
    try {
      ...
      return {
        ...
        signedCmd: null,
        errors:
          'Invalid response from WalletConnect. Try disabling the gas station and try again.',
      };
    } catch (error) {
